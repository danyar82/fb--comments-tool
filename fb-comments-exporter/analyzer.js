document.addEventListener('DOMContentLoaded', () => {
  ...
  function applyTranslation() {
    ...
    const standaloneBtnText = document.getElementById('exportStandaloneBtn')?.lastChild;
    if (standaloneBtnText && standaloneBtnText.nodeType === 3) standaloneBtnText.textContent = ' ' + t('standaloneExport');
  ...
  function loadSettings() {
    const data = JSON.parse(localStorage.getItem('fbce_settings') || '{}');
    sbUrlInput.value = data.sbUrl || 'https://qnmwdsigshpsshhdtkls.supabase.co';
    sbKeyInput.value = data.sbKey || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InFubXdkc2lnc2hwc3NoaGR0a2xzIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzQxNjkyNjEsImV4cCI6MjA4OTc0NTI2MX0.7WAtA0Gzk1pIfOirzO1ahiFmDuJRsXJdZ1jsV4Z4P6w';
  }
  loadSettings();
  ...
  saveSettingsBtn?.addEventListener('click', () => {
    ...
    localStorage.setItem('fbce_settings', JSON.stringify(settings));
  ...
  document.getElementById('shareLinkBtn')?.addEventListener('click', async () => {
    ...
    try {
      ...
      const exportData = {
        ...
        m: currentMasterSorted.slice(0, 1000), // More data allowed
        f: currentFilesSorted.map(file => ({
          name: file.name,
          total: file.total,
          sorted: (file.sorted || []).slice(0, 500)
        }))
      };

      // Read directly from inputs to support hardcoded values
      ...
      if (hasSupabase) {
        // Supabase logic
        const res = await fetch(`${sbUrl}/rest/v1/shares`, {
          method: 'POST',
          headers: {
        ...
        if (resData[0]?.id) {
          shareUrl = VIEWER_BASE_URL + '?sb=' + resData[0].id;
        } else {
          throw new Error('Supabase ID Error');
        }
      } else {
        // Npoint with Compression
        const jsonStr = JSON.stringify(exportData);
        const compressed = LZString.compressToEncodedURIComponent(jsonStr);

        // If it's small enough, we can even put it in URL directly, 
        // but for consistency let's stick to Npoint for now but send the compressed format

        const res = await fetch('https://api.npoint.io/bins', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
          body: JSON.stringify({ c: compressed }) // Send as compressed property
        });

        if (!res.ok) throw new Error('Npoint Error: ' + res.status);
        const resData = await res.json();
        if (resData.key) {
          shareUrl = VIEWER_BASE_URL + '?shared=' + resData.key;
        } else {
